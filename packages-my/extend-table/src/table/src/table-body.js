...
import ValidateRow from './validate/validate-row'
...
export default {
  ...
  methods: {
    clearRowValidate(index, props = []) {
      const row = this.$refs['validate' + index]
      if (!row) {
        return
      }
    ...
    clearValidate(props = []) {
      const rows = this.data.map((i, index) => {
        return this.$refs['validate' + index]
      }).filter(i => i)
      rows.forEach(row => {
        row.clearValidate(props)
      })
    ...
    validateRow(index, callback) {
      const validateRow = this.$refs['validate' + index]
      if (!validateRow) {
        return
      }
    ...
    validate(callback) {
      let result = []
      const validateRows = this.data.map((i, index) => {
        return this.$refs['validate' + index]
      }).filter(i => i)
      validateRows.forEach((validateRow, index) => {
        validateRow.validate((valid, invalidFields) => {
          result.push({
            index: index,
            valid: valid,
          ...
          if (result.length === validateRows.length) {
            const valid = result.every(i => i.valid)
            callback(valid, result.sort((a, b) => a.index < b.index))
          }
        })
      })
    },
